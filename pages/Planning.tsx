                                        ...
                                        {showClientSuggestions && clientSearchTerm && (
                                            <div className="absolute z-50 w-full mt-1 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-xl shadow-2xl max-h-60 overflow-y-auto">
                                                {filteredClientsModal.map(c => (
                                                    <div key={c.id} onClick={() => handleClientSelect(c)} className="p-3 border-b dark:border-slate-700 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer flex justify-between items-center">
                                                        <div className="min-w-0 pr-2">
                                                            <div className="font-bold text-sm truncate">{c.nome}</div>
                                                            <div className="text-[10px] text-gray-500 truncate">{c.indirizzo}</div>
                                                        </div>
                                                        {getClientStatus(c.id) && <span className={`text-[8px] font-extrabold px-1.5 py-0.5 rounded border whitespace-nowrap ${getClientStatus(c.id)!.color}`}>{getClientStatus(c.id)!.label}</span>}
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                                    {technicians.map(t => {
                                        ...
                                        return (
                                            <div key={t.id} onClick={() => toggleTechSelection(t.id)} className={`flex items-center p-2.5 border rounded-xl cursor-pointer transition-all ${isSelected ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20' : 'border-gray-100 dark:border-slate-700 hover:bg-gray-50'}`}>
                                                <div className="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs mr-3 shrink-0" style={{backgroundColor: t.color}}>{t.name.charAt(0)}</div>
                                                <span className="text-sm font-semibold flex-1">{t.name}</span>
                                                <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${isSelected ? 'bg-primary-600 border-primary-600' : 'border-gray-300'}`}>{isSelected && <CheckCircle size={12} className="text-white"/>}</div>
                                            </div>
                                        );
                                    })}
                                </div>
